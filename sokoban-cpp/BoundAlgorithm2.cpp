...
int BoundAlgorithm::greedyBound(int *boxArray, int pos)
{
    ...
    {
        b = boxPath(y.p, x.p); // berechnet Ort zu dem Leogolas muss um die Box in Richtung Ziel zu schieben => vor jeder Ecke erhöht sich die lower bound um +2. Trotzdem sinnvoll?
        // Ich vermute ja es hat aber definitiv auch nachteile
        //p = y.p;
        x = minimum(boxArray, boxCount, matchedBoxes, b); // Box point
