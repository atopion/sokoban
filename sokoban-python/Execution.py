...
class Execution:
    ...
    def possible_moves(self, box_array, cleared_map, pos):
        ...
        for a in [1, -1, self.width, -self.width]:
            ...
            b2 = not (self.cont(box_array, pos + a) != -1 and (self.cont(box_array, pos + 2*a) != -1 or cleared_map[pos + a] == 4))
    ...
    def analyse_state(self, node):
        ...
        pos = node.player_pos
        clearedMap = self.game_map.getClearedMap()

        moves = self.possible_moves(box_array, clearedMap, pos)
        for m in moves:

            # a new_box_array only needs to be changed if a box was moved
            i = self.cont(box_array, m[1])
            new_box_array = box_array
            if i != -1:
            ...
            bound = self.bound_algorithms.greedyBound(new_box_array, pos)

            if bound == 8:
                print("NODE: ", node, " MOVES: ", moves)
            ...
            if bound < node.lower_bound:
                print("ANALYSE: ", new_node, " BOUND: ", bound)
                res = self.analyse_state(new_node)
                if res is not None:
                    return res
            else:
                print("ADD TO SET: ", new_node, " BOUND: ", bound)
                self.openSet.add(new_node)

            '''f = True
            for j in range(0, len(node.sons), 1):
                if bound < node.sons[j].lower_bound:
                    node.sons.insert(j, Node(new_box_array, m[0], node, m, bound, node.depth +1))
                    # result.insert(j, Node(new_box_array, pos, node, m, bound))
                    f = False
                    break
            if f:
                node.sons.append(Node(new_box_array, m[0], node, m, bound, node.depth + 1))'''
